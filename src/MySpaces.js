...
class MySpaces extends Component {
  ...
  componentDidMount() {
    firebase
      ...
      .on("value", snapshot => this.setState({ spaces: snapshot.val() }));
  }
        ...
        arr.push(
          <Panel className="listingPanel">
            <SpaceListing
              ...
              editSpace={this.props.editSpace.bind(this)}
            />
          </Panel>
        );
      }
  ...
  render() {
    return (
      <div>
        <h1>MY SPACES</h1>
        <Link to="/spaces/add" className="profile-button btn btn-default">
          Add A New Space
        </Link>
        {this.renderSpaces()}
      </div>
