...
class OrderDetails extends Component {
  ...
  componentDidMount() {
    firebase.database().ref('spaces/' + this.props.space.spaceId).on('value', function(snapshot) {
      this.setState({ location: { lat: snapshot.val().lat, lng: snapshot.val().lng}});
    }.bind(this));
  }
  ...
  render() {
    ...
    if (this.props.space.climate_control) {
      climate_control_local = [<p className="p-body"> Has Climate Control System </p>];
    }
    ...
    return (
      <Panel className="orderPanel">
        ...
        <Col md={4}>
          ...
          <p className="p-body">{this.props.space ? this.props.space.address : null}</p>
        ...
        <Col md={4}>
          ...
          <p className="p-body">{moment(this.props.endDate).diff(moment(this.props.startDate), 'days') + 1} Day(s)</p>
          <p className="p-title">Size</p>
          <p className="p-body">{this.props.space ? this.props.space.size : null}</p>
